...
mov <- read.csv("HomeworkChallengeDataset.csv")
...
p + (aes(size = Budget...mill.,  colour = Studio)) + geom_boxplot(alpha = 0.7) 
...
p + (aes(size = Budget...mill., colour =Studio)) + geom_boxplot(alpha = 0.7, outlier.colour = NA) 


#Let's "Save" our progress by placing it into a new object:

q <- p + (aes(size = Budget...mill., colour =Studio)) + geom_boxplot(alpha = 0.7, outlier.colour = NA) 
